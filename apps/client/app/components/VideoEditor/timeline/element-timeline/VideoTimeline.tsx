...
export default function VideoTimeline() {
  ...
  const { mediaFiles, activeElement, activeElementIndex, timelineZoom } = useAppSelector(
  ...
  const handleLeftResize = (clip: MediaFile, target: HTMLElement, width: number) => {
    ...
    const constrainedLeft = Math.max(
      clip.positionStart + (clip.positionEnd - clip.positionStart - newPositionStart),
      0
    );
        ...
        .map((clip) => (
          <div key={clip.id}>
            ...
            <Moveable
              ...
              container={null}
              renderDirections={
                activeElement === "media" && mediaFiles[activeElementIndex].id === clip.id
                  ? ["w", "e"]
                  : []
              }
              draggable={true}
